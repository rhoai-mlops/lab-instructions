...
1. OpenShift magic provides a great way to collect logs across services, anything that's pumped to STDOUT or STDERR is collected and added to LokiStack. This makes indexing and querying logs very easy. Let's take a look at OpenShift Logs UI now: in the `OpenShift Console`, go to `Observe` > `Logs` tab and choose your test project.
