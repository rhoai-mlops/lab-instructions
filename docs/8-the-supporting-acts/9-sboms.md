...
8. After the task successfully finishes, go to `OpenShift UI` > `Builds` > `ImageStreams` and select `jukebox`. In the `History` tab, you'll see a tag ending with `.sbom` and `.att` which shows you that an attestation for the SBOM predicate is attached. With this, the SBOM is signed (and therefore tamper-proof) as it is within an attestation, and consumers can validate its authenticity.
